...
**Current Verified Baseline (2026-07-05)**: 287 strict-valid unique records (all with raw refs; raws present verified; post PII-rebuild + dedup + fresh per guide). 
- Locanto Peru (hombre busca mujer real): 145
- Doplim Peru (hombre busca mujer or Contactos): 108
- Facebook Public (indexed / search-first / groups): ~30
- Other indexed public: 4
- 521 raw `.html` archives in `data/raw/ads/`
...
- All collection via search-first + full gates + strict filters per this guide ONLY.
- Fresh web_search + list_extract on clean listings + short direct batches: low incremental yield (dups + inter + filter skips dominant); public indexed supply limited. Honest exhaustion after documented high effort (100+ attempts, 50+ searches, 30+ directs/wave, city sweeps, multiple storms).
