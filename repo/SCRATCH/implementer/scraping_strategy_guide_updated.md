...
**Current Verified Baseline (2026-07-05)**: 274 strict-valid unique records (all with raw refs; raws present verified). 
- Locanto Peru (hombre busca mujer real): 138 (132 male-offering perspective tagged)
- Doplim Peru (hombre busca mujer or Contactos): 102
- Facebook Public (indexed / search-first / groups): ~30
- Other indexed public: 4
- 480 raw `.html` archives in `data/raw/ads/`
...
## Additional Recent Learnings (2026-07-05, from fresh_doplim_push / more_doplim / mixed_push / fb_fresh / extra_fresh / list_fresh bg batches per guide)
...
- List extract on clean listings (per guide secondary): on clean Arequipa Hombre-busca-mujer listing from search, extracted 48 /ID_ links (high yield). Subsequent detail fetches with full gates/filters/redact/dedup/PII led to net adds (volume reached 274). Still, many dups in fetched details.
- Confirms: Must compile 30-50+ **fresh** seeds per push via repeated web_search (exact title phrases + city variants + "ID_" / "id-"); pre-load dedup critical; shorter batches essential (EPIPE risk + diminishing). List extract on clean (len>2500 !inter) renders is high-potential for volume when directs dry up.
...
- Guided bg batches (fresh_doplim_push, more_doplim, mixed_push, extra_fresh_doplim, fb_fresh_batch, list_fresh using exact recipe): 
  ...
  - list_fresh on clean Arequipa Hombre-busca-mujer listing: extracted 48 /ID_ (high yield); contributed to net adds after full gates (volume to 274).
- Current: 274 total (Locanto 138 platform / 132 male H; Doplim 102; FB ~30), 480 raws, PII=0 (_contains), all referenced raws present, gate --phase 4 PASSED.
- Proofs saved to REAL_SCRATCH/implementer/ (reassess_state_*.log, state_reassess_*.json, *_doplim_*.log, fb_fresh_*.log, mixed_*.log, list_fresh_*.log, pytest_*, gate_*.txt) + SCRATCH/implementer/.
