...
**Current Verified Baseline (2026-07-05)**: 276 strict-valid unique records (all with raw refs; raws present verified; post PII-rebuild from raw). 
- Locanto Peru (hombre busca mujer real): 134
- Doplim Peru (hombre busca mujer or Contactos): 108
- Facebook Public (indexed / search-first / groups): ~30
- Other indexed public: 4
- ~482 raw `.html` archives in `data/raw/ads/`
- Gate `phase_gate.py --phase 4`: **PASSED** (PII=0 via _contains on full record, no dups, no inters, raw coherence, required fields)
...
## Additional Recent Learnings (2026-07-05, from fresh_doplim_push / more_doplim / mixed_push / fb_fresh / extra_fresh / list_fresh / persevere_push / persevere_expanded bg batches per guide)

- Latest short bg batches (search-first directs + full UA/waits/len+anti-inter gate + TARGETS+MALE + pre-dedup + redact + PII gate): 90-100%+ dups or "inter"/"short"/"filter" skips on repeated/known seeds. Net 0 added in dedicated persevere_push/expanded runs (all dups mostly). Doplim yields occasionally on brand-new id- seeds; Locanto directs rarely after initial.
- Locanto: extremely high "dup skip" + "inter gate skip" even on /ID_ surfaced recently. 
- Doplim: more reliable for incremental when fresh id- from web_search.
- FB: low conversion, many "no filter".
- **List extract on clean listings (secondary per guide)**: clean Arequipa Hombre-busca-mujer listing from search yielded extraction of 48 /ID_ links (high yield). Detail fetch with full gates/filters contributed to reaching 276. Use ONLY after clean render (len>2500 !inter).
- Confirms: Repeated directs from prior = dead-end. **Every wave requires fresh web_search** (exact phrases from good titles + new city variants + ID_/id-). Compile 30-60+ directs. Pre-load rids. Short batches (5-8 cap) for stability. 
...
- Exhaustion: public indexed supply very limited; high effort (100+ att/50+ searches/storms/30+ directs/wave) yields slow adds or 0 (dups). Persevere only with fresh discovery + list-on-clean or honest doc with proof. Never retry dead-ends.
- **Critical lesson from inline bg appends**: Some collector one-offs (python -c) used partial PII check (only on t/b dict) allowing PII to leak into metadata (seed_text, old snippets). Gate on *full record json* caught it. Fix: strict rebuild re-extract title/body ONLY from raw + sanitize metadata to safe keys only + _contains on complete constructed rec before append. 3+ bad dropped, clean 276 gate-pass post-rebuild. Always prefer full tools or replicate exact gate on final record.
...
- Guided bg batches (fresh_*/more_*/persevere_push/persevere_expanded + list_fresh using exact recipe): 
  - Logs show: mostly "dup skip", "inter", "short", "filter", "no target+male".
  - Net adds: 0 in latest persevere_push/expanded (heavy dups on known directs); prior waves +1-3 on fresh seeds (Doplim/FB). list_fresh clean Arequipa extract 48 /ID_ contributed incremental after full gates.
- Current: 276 total (Locanto 134; Doplim 108; FB ~30; other 4), ~482 raws, PII=0 (_contains on full), all referenced raws present, gate --phase 4 PASSED (post final rebuild from good raws only).
- Proofs saved to REAL_SCRATCH/implementer/ (persevere_*.log, list_fresh*.log, *_doplim_*.log, gate_*.txt, pytest_*.txt) + SCRATCH/implementer/.
...
**Future agents**: Follow this document exactly. All dead-ends (listings-first, weak gates, no search-first, no dedup, no PII, legacy no-raw, long batches, repeated directs without fresh search) already catalogued and discarded. Search-first + full Playwright gates (12 UA, es-PE, 30s waits, len+keyword+inter gate, retries/backoff) + TARGETS+MALE + redact + _contains + raw archive + pre-dedup + append main ONLY + list extract on clean renders ONLY = the sole path that produced the clean 276. Persevere with fresh seeds + list-on-clean or document exhaustion with proof. Never retry discarded approaches.